...
function PopoverHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-col gap-0.5 text-[11px] font-[family-name:var(--mono)]", className)}
      {...props}
    />
  )
}
...
function PopoverTitle({ className, ...props }: PopoverPrimitive.Title.Props) {
  return (
    <PopoverPrimitive.Title
      ...
      className={cn("font-semibold font-[family-name:var(--pixel)] text-xs", className)}
      {...props}
    />
  )
}
